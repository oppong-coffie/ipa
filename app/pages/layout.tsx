...
export default function PagesLayout({
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[#3e2723] text-[#f5f5dc]">
      {/* --- Header / Navbar --- */}
      <motion.header
        initial={{ y: -100 }}
        animate={{ y: 0 }}
        ...
        className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
          isScrolled ? "bg-[#4e342e]/85 backdrop-blur-sm shadow-md" : ""
        }`}
      >
        <nav className="flex items-center justify-between px-4 py-3 sm:px-6 md:px-8 lg:px-12">
          ...
          <div className="flex items-center space-x-2 sm:space-x-3">
            ...
            <div className="leading-tight">
              <div className="font-bold text-base sm:text-lg text-[#f5f5dc]">
                IPA
              </div>
            ...
            {navigationItems.map((item, i) => (
              <motion.a
                ...
                className={`transition-colors duration-200 text-sm xl:text-base ${
                  item.name === "Home"
                    ? "text-yellow-500"
                    : "text-[#f5f5dc] hover:text-yellow-500"
                }`}
              >
                {item.name}
              </motion.a>
        ...
        {isMenuOpen && (
          <motion.div
            ...
            className="lg:hidden bg-[#3e2723]/95 backdrop-blur-sm"
          >
            <div className="px-4 sm:px-6 py-6 space-y-4">
              {navigationItems.map((item) => (
                <a
                  key={item.name}
                  href={item.href}
                  className="block text-[#f5f5dc] hover:text-yellow-500 transition-colors duration-200 text-base sm:text-lg"
